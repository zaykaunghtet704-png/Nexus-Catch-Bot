...
from waifu import CHARA_CHANNEL_ID, OWNER_ID, application, collection, db, sudo_users
from waifu.config import Config

RARITY_MAP = Config.RARITY_MAP
RARITY_STRS = {str(v).lower(): str(v) for v in RARITY_MAP.values()}
...
def _is_sudo(uid: int) -> bool:
    ...
    return uid in sudo_list or uid == int(OWNER_ID) if OWNER_ID else False
...
def _char_caption(char: dict, uploader_id: int, uploader_name: str) -> str:
    ...
    rarity = escape(str(char.get("rarity", "Common")))
    anime = escape(str(char.get("anime", "Unknown")))
    cid = escape(str(char.get("id", "0000")))
    uname = escape(str(uploader_name or "Admin"))
    ...
    return (
        ...
        f"🌸 <b>Anime:</b> {anime}\n"
        f"🌱 <b>ID:</b> {cid}\n\n"
        f"Added by <a href='tg://user?id={uploader_id}'>{uname}</a>"
    )
...
async def upload(update: Update, context: CallbackContext) -> None:
    if not update.effective_user or not _is_sudo(update.effective_user.id):
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ <b>{escape(name)}</b> added! ID: <code>{char_id}</code>",
            parse_mode=ParseMode.HTML,
        )
    except Exception as e:
        await update.message.reply_text(
            f"❌ Channel post failed: {escape(str(e))}\nCharacter was <b>not</b> saved.",
            parse_mode=ParseMode.HTML,
        )
...
async def uploadchar(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await collection.insert_one(char)
        await update.message.reply_text(
            f"✅ <b>{escape(parsed['name'])}</b> uploaded!\n"
            f"🎴 Rarity: {escape(parsed['rarity'])}\n"
            f"📺 Anime: {escape(parsed['anime'])}\n"
...
async def delete(update: Update, context: CallbackContext) -> None:
    ...
    if len(context.args) != 1:
        await update.message.reply_text("Usage: <code>/delete ID</code>", parse_mode=ParseMode.HTML)
        return
    ...
    await update.message.reply_text(
        f"✅ Deleted <b>{escape(char.get('name', 'Character'))}</b> (<code>{char_id}</code>)",
        parse_mode=ParseMode.HTML,
    )
...
async def update_char(upd: Update, context: CallbackContext) -> None:
    if not upd.effective_user or not _is_sudo(upd.effective_user.id):
    ...
    try:
        ...
        await upd.message.reply_text(f"⚠️ DB updated but channel sync failed: {escape(str(e))}")
        return

    await upd.message.reply_text(
        f"✅ <b>{escape(char.get('name', 'Character'))}</b> — <code>{field}</code> updated.",
        parse_mode=ParseMode.HTML,
    )
