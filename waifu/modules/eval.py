"""
modules/eval.py — Python code evaluator and shell execution module for devs.
"""
import asyncio
import io
import textwrap
...
async def _send(text: str | None, update: Update, bot) -> None:
    ...
    # Safe check for forum thread id
    is_forum = getattr(update.effective_chat, "is_forum", False)
    tid = update.effective_message.message_thread_id if is_forum else None
...
def _dev(fn):
    async def w(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not update.effective_user or update.effective_user.id not in DEV_LIST:
            return
...
async def shell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = update.message.text.split(" ", 1)
    if len(parts) < 2:
        await _send("No command.", update, context.bot)
        return
    try:
        ...
        await _send("❌ Command timed out (15s limit).", update, context.bot)
    except Exception as e:
        await _send(f"❌ Shell error: {e}", update, context.bot)
