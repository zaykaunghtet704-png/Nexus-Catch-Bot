...
async def trade(update: Update, context: CallbackContext) -> None:
    ...
    a_char = next((c for c in (a_doc or {}).get("characters", []) if str(c.get("id")) == str(context.args[0])), None)
    b_char = next((c for c in (b_doc or {}).get("characters", []) if str(c.get("id")) == str(context.args[1])), None)

    if not a_char:
        await update.message.reply_text("❌ You don't own that character.")
        return
    if not b_char:
        await update.message.reply_text("❌ They don't own that character.")
        return
    ...
    await update.message.reply_text(
        ...
        f"<a href='tg://user?id={a.id}'>{escape(a.first_name or 'User')}</a> offers "
        f"<b>{escape(a_char.get('name', 'Character'))}</b>\n"
        f"for <b>{escape(b_char.get('name', 'Character'))}</b> from "
        f"<a href='tg://user?id={b.id}'>{escape(b.first_name or 'User')}</a>\n\n"
        f"<i>{escape(b.first_name or 'User')}, accept?</i>",
        parse_mode=ParseMode.HTML,
        reply_markup=kb,
    )
...
async def trade_cb(update: Update, context: CallbackContext) -> None:
    ...
    if action == "trade_no":
        await q.edit_message_text("❌ Trade declined.")
        return

    a_id, b_id = state["a_id"], state["b_id"]
    a_char, b_char = state["a_char"], state["b_char"]

    # Re-verify character ownership before completing trade
    a_doc = await user_collection.find_one({"id": a_id, "characters.id": a_char["id"]})
    b_doc = await user_collection.find_one({"id": b_id, "characters.id": b_char["id"]})

    if not a_doc or not b_doc:
        await q.edit_message_text("❌ Trade failed! One of the characters is no longer in inventory.")
        return
    ...
    await q.edit_message_text(
        f"✅ Trade complete!\n\n"
        f"<a href='tg://user?id={a_id}'>{escape(state['a_name'] or 'User')}</a> ← <b>{escape(b_char.get('name', ''))}</b>\n"
        f"<a href='tg://user?id={b_id}'>{escape(state['b_name'] or 'User')}</a> ← <b>{escape(a_char.get('name', ''))}</b>",
        parse_mode=ParseMode.HTML,
    )
...
async def gift(update: Update, context: CallbackContext) -> None:
    ...
    a_doc = await user_collection.find_one({"id": a.id})
    char = next((c for c in (a_doc or {}).get("characters", []) if str(c.get("id")) == str(context.args[0])), None)
    if not char:
    ...
    await update.message.reply_text(
        f"🎁 Gift <b>{escape(char.get('name', 'Character'))}</b> to "
        f"<a href='tg://user?id={b.id}'>{escape(b.first_name or 'User')}</a>?",
        parse_mode=ParseMode.HTML,
        reply_markup=kb,
    )
...
async def gift_cb(update: Update, context: CallbackContext) -> None:
    ...
    state = _gifts.get(gid)
    if not state:
        await q.answer("⌛ Gift expired!", show_alert=True)
        return
    if q.from_user.id != state["a_id"]:
        await q.answer("❌ Not your gift!", show_alert=True)
        return
    ...
    if action == "gift_no":
        await q.edit_message_text("❌ Gift cancelled.")
        return

    char = state["char"]

    # Re-verify character ownership
    a_doc = await user_collection.find_one({"id": state["a_id"], "characters.id": char["id"]})
    if not a_doc:
        await q.edit_message_text("❌ Gift failed! Character is no longer in your inventory.")
        return
    ...
    if b_doc:
        ...
        await user_collection.insert_one({
            "id": state["b_id"],
            "username": state["b_username"],
            "first_name": state["b_name"],
            "characters": [char],
            "coins": 0,
            "xp": 0,
            "wins": 0,
    ...
    await q.edit_message_text(
        f"✅ <b>{escape(char.get('name', ''))}</b> gifted to "
        f"<a href='tg://user?id={state['b_id']}'>{escape(state['b_name'] or 'User')}</a>!",
        parse_mode=ParseMode.HTML,
    )
