...
from waifu import PHOTO_URL, application, user_collection
...
def _calc_level(xp: int) -> tuple[int, int, int]:
    """Returns (level, xp_into_level, xp_needed). Safe non-blocking logic."""
    if xp <= 0:
        return 1, 0, _xp_for_level(2)
    ...
    return level, max(0, xp - floor), max(1, nxt - floor)
...
async def profile(update: Update, context: CallbackContext) -> None:
    ...
    # Unique characters dictionary map
    unique_chars_map = {c["id"]: c for c in chars if isinstance(c, dict) and "id" in c}
    unique_count = len(unique_chars_map)
    total_count = len(chars)

    # Rarity breakdown
    rarity_count: dict[str, int] = {}
    for c in unique_chars_map.values():
    ...
    photo: str | None = None
    if fav_id:
        fav_char = unique_chars_map.get(fav_id)
        if fav_char:
            photo = fav_char.get("img_url")
    ...
    if photo:
        try:
            ...
            pass  # Fallback to text on photo upload fail
