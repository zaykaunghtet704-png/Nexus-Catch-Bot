...
async def active_users_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await is_admin_or_owner(update.effective_user.id): return
    now = datetime.utcnow()
    a24h = await users_col.count_documents({"last_active": {"$gte": now - timedelta(hours=24)}})
    a7d = await users_col.count_documents({"last_active": {"$gte": now - timedelta(days=7)}})
    total = await users_col.count_documents({})
    rate = (a7d / total * 100) if total > 0 else 0
    await update.message.reply_text(f"📊 <b>Analytics</b>\nUsers: <b>{total:,}</b>\n24h Active: <b>{a24h:,}</b>\n7d Retention: <b>{rate:.2f}%</b>", parse_mode=ParseMode.HTML)
...
async def generate_gift_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await codes_col.update_one({"code": code_name}, {"$set": {"coins": coins, "max_uses": max_uses, "used_count": 0, "created_at": datetime.utcnow()}}, upsert=True)
...
async def broadcast_pin_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await is_owner(update.effective_user.id) or not update.message.reply_to_message:
        await update.message.reply_text("⚠️ Reply to a message with <code>/broadcastpin</code>", parse_mode=ParseMode.HTML)
        return
    ...
    for chat in chats:
        try:
            sent = await update.message.reply_to_message.copy(chat_id=chat.get("chat_id"))
            try: await context.bot.pin_chat_message(chat_id=chat.get("chat_id"), message_id=sent.message_id)
            except Exception: pass
            success += 1
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. INTERACTIVE ADMIN DASHBOARD & REGISTRY
# ══════════════════════════════════════════════════════════════════════════════
